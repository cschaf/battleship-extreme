...
public class LocalClientController implements Serializable {
				...
				ActionListener fieldListener = new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							if (turnMade) {
								ctrl.appendGameLogEntry("Player " + attackingPlayerName + " attacked " + attackedPlayerName + " " + orientation + " with ship " + currentShip.getType().toString()
										+ " at start Field X:" + xPos + "  Y: " + yPos);
							}
	...
	private void makeAiTurn() {
		try {
			...
			ctrl.appendGameLogEntry("Player " + game.getCurrentPlayer().getName() + " attacked " + gui.getPanelGame().getComboBoxEnemySelection().getSelectedItem().toString() + " "
